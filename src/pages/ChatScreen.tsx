      ...
      <header className="px-4 py-3 border-b border-border flex items-center gap-3 bg-background/80 backdrop-blur-md sticky top-0 z-10">
        ...
        <div className="relative cursor-pointer" onClick={() => setPreviewAvatar(isGroup ? chatPartner?.thumbnail_url : chatPartner?.avatar_url)}>
          ...
          {!isGroup && isOnline(chatPartner?.last_seen) && (
            <div className="absolute bottom-0 right-0w-3 h-3 bg-green-500 border-2 border-background rounded-full"></div>
          )}
        </div>
...
<dyad-chat-summary>Fixing message wrapping in chat screen</dyad-chat-summary><dyad-write path="src/pages/ChatScreen.tsx" description="Fixing message wrapping and horizontal overflow in the chat screen.">
"use client";
